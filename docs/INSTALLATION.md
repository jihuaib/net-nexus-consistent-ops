...
13. 确定性规则引擎识别根因和派生异常，输出稳定诊断链。
14. Syslog、Trap、Telemetry 按类型拆分页面，通过 WebSocket 实时展示原始输入、接收状态和时间窗口关联摘要。
15. FRR lab 接口真实 down/up 时，容器内链路事件 agent 会发出 UDP Syslog 和标准 SNMPv2 linkDown/linkUp Trap。

当前尚未实现：

1. 第三阶段完整 Spine-Leaf 三台以上设备联动诊断。
2. NETCONF、Telemetry 长连接接收器和生产级事件总线。
3. Redis、数据库和 RAG 知识库。
...
```bash
cd /Users/jihuaibin/code/NetNexusConsistentOps/frontend
VITE_API_BASE=http://127.0.0.1:8010 npm run dev -- --host 127.0.0.1 --port 5178
```
...
```bash
VITE_API_BASE=http://127.0.0.1:8011 npm run dev
```
...
| 多异常 Fact | 先上报链路/BGP/路由/FIB/业务事件后，`/api/facts` 包含 `SYSLOG_LINK_DOWN`、`BGP_NEIGHBOR_DOWN`、`ROUTE_MISSING`、`FIB_ENTRY_MISSING`、`SERVICE_UNREACHABLE` |
| 多异常诊断 | `/api/agent/chat` 返回 `INTERFACE_DOWN_CAUSES_BGP_ROUTE_LOSS` |
...
6. 增加 Redis、数据库和 RAG 知识库作为生产化增强。
